...
std::cout << "[ DEBUG ] " __FILE__ << ":" << __LINE__ << " " << std::format(__VA_ARGS__);
...
std::cout << FYEL("[ INFO ] ") << __FILE__ << ":" << __LINE__ << " " << label << " - " << expr << std::endl;

#define test_debug(expr) \
std::cout << FBLU("[ TEST ] ") << __FILE__ << ":" << __LINE__ << " " << expr << std::endl;
...
test_debug(BOLD("Running test " #func))\
if (func()) { \
    test_debug(FGRN("Test " #func " passed.")); \
} else { \
    test_debug(FRED("Test " #func " failed.")); \
}
