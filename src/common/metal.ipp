...
MetalDevice::MetalDevice(){
    ...
    _map_cmd_q->setLabel(NS::String::string("mtl_device", NS::UTF8StringEncoding));
...
MTL::Buffer *MetalDevice::get_buffer(size_t index) {
...
bool MetalDevice::request_buffer(uint8_t id, const void * begin, size_t size) {
    ...
    _clear_buffer_index(id);
    auto buffer = _mtl_device->newBuffer(begin, size, MTL::ResourceStorageModeShared);
    if (!buffer) return false;
    _buffers[id] = buffer;
    return true;
}
